...
export async function searchProducts(
  ...
  data: Array<{ id: string; nombre: string; sku: string; tipo_unidad: string; unidad_base: string; factor_conversion: number; precio_venta: number; stock_actual: number; stock_minimo: number; categoria: string }> | null
  error: string | null
}> {
  const session = await getSession()
  if (!session.data) {
  ...
  const { data, error } = await supabase
    .from("productos")
    .select("id, nombre, sku, tipo_unidad, unidad_base, factor_conversion, precio_venta, stock_actual, stock_minimo, categoria")
