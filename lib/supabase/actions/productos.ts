...
export async function searchProducts(
  ...
  data: Array<{ id: string; nombre: string; sku: string; tipo_unidad: string; unidad_base: string; factor_conversion: number }> | null
  error: string | null
}> {
  const session = await getSession()
  if (!session.data) {
  ...
  const { data, error } = await supabase
    .from("productos")
    .select("id, nombre, sku, tipo_unidad, unidad_base, factor_conversion")
